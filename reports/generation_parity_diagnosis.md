...
日期：2026-09-05。关联 M-02/M-08、SPEC Step 060～064及073～078。ADR-019已按用户BF16原因条件授权接受，本机SM86独立工程确认通过；旧失败保留，不等于真实Agent/G1通过。
...
flowchart TD
    ...
    MATCH --> EXACT
    EXACT --> ADR[ADR-019 已授权；本机独立工程确认通过]
```
...
原生部署继续使用原BF16默认计算，既不填充矩阵行，也不启用FP64投影或reduction-off。M0仍需真正的Agent loop，G1仍需真实环境结果，不能由本工程确认取代。

## 独立确认结果：本机 M-02 通过

配置在commit `cb15c56` 冻结；首轮入口因漏设首token reset被官方assert拒绝，尚未计算logits。修正harness及其回归测试后，以commit `d38e6ed` 运行，提示、seed、内部reset和阈值均不变，旧入口失败文件保留。

| 检查 | 0.4B | 1.5B |
|---|---:|---:|
| 严格同形状 logits / 三类状态逐值相同 | 6/6 | 6/6 |
| 原生长窗 case 通过 | 9/9，最长8K | 9/9，最长16K |
| 末尾受检位置总数 | 576 | 576 |
| 最坏 mean KL / p95 KL | 0.001081 / 0.004516 | 0.001250 / 0.004572 |
| 最低 top1 一致率 | 95.3125% | 95.3125% |
| 高置信预测翻转 / 受检高置信位置 | 0/222 | 0/264 |
| K0 / 生成重复性 | 全部通过 | 全部通过 |
| 工程检查耗时 | 30.72s | 38.14s |
| 工程检查 peak allocated | 9,662,005,248 bytes | 11,859,980,288 bytes |

上述数值实际也落在旧数值上限以内；但测试的是实际128-token prefill后的64-token continuation，不是旧冷启动全程逐token，故不能据此改写原失败。峰值包含整段reference和比较张量，不能当训练或部署常驻显存。

[机器汇总及原始报告SHA](generation_confirmation_summary.json)保存全部分母、最坏指标和数值开关；完整报告在data root。本机数值阻塞已解除，下一步是**真实8K tiny smoke、32/128 overfit、GPU checkpoint恢复及Agent loop**。远程SM89/DDP尚未验收，没有自动放行规模化训练或M2。
